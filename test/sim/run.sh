...
#
# Each scenario drives the real esp32_sim firmware through inputs (buttons, the
# FLOW switch) and asserts actuator pin states with expect-pin -- no serial, no SPA.
# This is the automatable replacement for eyeballing serial: wokwi-cli's serial
# stream is broken against the current API (a `bytes` field parse crash, the
# upstream "Blank Terminal Problem"), and [[net.forward]] is a VS Code-extension
# feature the CLI does not implement. Pin assertions sidestep both.
#
