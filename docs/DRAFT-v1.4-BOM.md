...
| Relay module, 1-channel opto | 2 (sold as a 4-pack; 2 spare) | 5V VCC, opto-isolated, **H/L jumper set HIGH**, triggers at 3.3V, relay = SRD-05VDC-SL-C, 10A contacts | Amazon — "1 channel 5V relay module optocoupler high/low trigger" | **v1.5 — the pump-switching relays.** Two **independent** boards (pump + safety) — better than a shared 2-channel board for the two-key chain (a fault on one can't take the other). Each: `VCC`→5V, `GND`→common, `IN`←control pin, contacts `COM`/`NO` in series on the pump feed. Safety `IN`←ATtiny pin 6, pump `IN`←ESP32 pin 22. **Set the jumper HIGH** (boot-LOW = off = fail-dry) and **bench-confirm the 3.3V trigger** before wiring the pump. Retires the bare SRD-24VDC-SL-C (drawer); no discretes to solder. |
