...
| Watchdog support passives | per ATtiny | 3× 10k + 1× 100nF | — | **10k pin7→GND** (heartbeat pull-down — safety-critical: a broken heartbeat must read as quiet so the pump disarms); **10k pin36→3.3V** + **100nF pin36→GND** (trip-line pull-up + filter); **10k RESET→3.3V** (glitch immunity). |
