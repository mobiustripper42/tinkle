...
| Relay module, 2-channel | 1 | **5V VCC, active-HIGH ("high-level") trigger, 3.3V-compatible, ≥10A contacts** | Amazon — "2 channel relay module high level trigger 3.3V" | **v1.5 — replaces the bare SRD + discrete driver.** Two relays + their transistor drivers + flyback, all on one board. `VCC`→5V, `GND`→common; `IN1`←ESP32 pin 22 (pump), `IN2`←ATtiny pin 6 (safety); the two contact sets go in series on the pump feed. **Active-HIGH is required** so boot-LOW = relays off = fail-dry. Retires the bare SRD-24VDC-SL-C (drawer). No discrete transistors/diodes needed. |
