...
- Inline fuse (~10A) on 24V output, TVS across 24V, reverse-polarity protection, a per-FET TVS (**1.5KE30CA, through-hole**) on each valve channel, flyback on the pump-relay coil (brownout insurance — mains here is brownout-prone).
...
7. ~~**Valve driver**~~ — **DONE:** discrete IRLZ44N low-side FET per valve + **1.5KE30CA (through-hole) TVS** drain-to-source. No H-bridge. Bare FETs + socketed ATtiny on an ElectroCookie protoboard; ×2 relay modules (pump + safety).
