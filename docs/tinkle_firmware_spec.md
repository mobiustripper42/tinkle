...
- **Withheld `CAL_RANGE` reject is intentionally trace-less (#158, closed won't-fix).** #144 correctly *withholds* the `FAULT_CAL_RANGE` latch when `finish()` would otherwise hit an unrelated in-flight run (an out-of-range calibration submitted while a scheduled/manual run is `Running`). In that path the rejected calibration leaves **no** durable trace — no latch, no RunLog row, no fault-log entry — only the live HTTP `Rejected` to the SPA. Accepted as-is: calibration is an interactive operator action with immediate on-screen feedback, so a durable breadcrumb is low value, and routing it through `FaultManager::note()` would need a `FaultManager&` wired into `CalibrationController` for a corner case. The live rejection is the signal.
...
- **Implemented (#55/#56/#57, Unit C / DEC-016).** Policy and plumbing are split on the platform line:
  ...
  - **`GET /api/status` additions** beyond the list above: fault-log ring (§14), `valveRestFlags` (DEC-014), calibration phase, schedule-drop counter, wifi mode/ip/rssi, uptime, and `distSummary` (DEC-025 — per-zone cycles completed vs planned-by-now + metered `centigallons` delivered; present only when Distributed Watering is active and the clock is valid; feeds the Home "Today · distributed" card).
...
- Fault codes: `FAULT_NO_FLOW`, `FAULT_UNEXPECTED_FLOW`, `FAULT_CAL_RANGE`, `FAULT_CLOCK` (optional/non-blocking). **Non-latching codes:** `FAULT_WATCHDOG` (DEC-023 — aborts the current run, preserves the queue; the relay is the safety) and `FAULT_VALVE_REST` (DEC-014 — log-only maintenance flag). Both are structurally refused by `raiseFault`. `MissedCycle` (DEC-025 — a Distributed-Watering cycle whose window elapsed with no run) goes one further: it never touches `FaultManager` at all. The `dist_summary` advisor writes it as a `Faulted` **RunLog** entry (`faultCode=MissedCycle`, 0 duration/volume), so it persists and telemeters through the Poop Deck publisher exactly like a faulted run (`"fault":"missed-cycle"`) — reaching Grafana without a run to ride on. Non-latching by construction (a log row commands no actuation); idempotent + reboot-safe because the persisted entry is the dedup marker.
...
- **Per-zone Distributed config — not built (#151, closed for complexity).** Distributed Watering (DEC-024) fires **all live zones** each cycle in a fixed compile-time firing order (`kFireOrder`, `scheduler.cpp`). Making that order operator-configurable, or adding a per-zone enable/disable toggle, was scoped in #151 and closed unbuilt: turning a single zone off inside an even-spread model is confusing — the plan would recompute on the enabled-zone count, silently changing every cycle's length and soak — and the permutation-order UI adds surface out of proportion to the benefit on a three-zone box. Zones are included/excluded by their plumbing + the runtime `zoneCount`, not a runtime toggle. (The disposition of the temporary session-23 flow-spike diagnostic value in `kFireOrder` — keep vs. revert to natural order — is a separate one-line follow-up, see #147.)
